...
function getNextFriday(date) {
  ...
  return new Date(
    answer.setDate(
      date.getDate() + (date.getDay() < 5 ? 5 : 12) - date.getDay()
    )
  );
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return 1 + (new Date(dateEnd) - new Date(dateStart)) / (3600 * 1000 * 24);
}
...
function formatDate(date) {
  ...
  return answer.toLocaleDateString('en-US', {
    hour: 'numeric',
    minute: '2-digit',
    second: '2-digit',
  });
...
function getCountWeekendsInMonth(month, year) {
  ...
  day = day === 0 ? 7 : day - 1;
  let answer = day > 6 ? 1 : 2;
  days -= 7 - day;
  answer += Math.floor(days / 7) * 2;
  answer += days % 7 > 5 ? (days % 7) - 5 : 0;
  return answer;
}
...
function getWeekNumberByDate(date) {
  const answer =
    new Date(date).getTime() - new Date(date.getFullYear(), 0, 1).getTime();
  return Math.ceil(
    (answer / 86400000 + 1 + new Date(date).getTimezoneOffset() / 1440) / 7
  );
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const begin = new Date(period.start.split('-').reverse());
  ...
  while (begin <= end) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (begin > end) break;
      answer.push(
        `${String(begin.getDate()).padStart(2, '0')}-${String(begin.getMonth() + 1).padStart(2, '0')}-${String(begin.getFullYear()).padStart(4, '0')}`
      );
      begin.setDate(begin.getDate() + 1);
    }
    begin.setDate(begin.getDate() + countOffDays);
  }
  return answer;
}
